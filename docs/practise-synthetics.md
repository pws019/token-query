...
- [x] 1. 在 `infra/cdk/lib/monitoring-stack.ts` 里新建一个共用的 SNS Topic（例如 `token-query-ops-alerts`），后续所有告警都往这一个 Topic 发，不用每类告警各建一个
- [x] 2. 给 Topic 加至少一个 Email 订阅（CDK 里订阅创建后依然要去邮箱点确认链接，这一步是异步的、CDK 部署完不代表订阅已生效）
- [x] 3. 把 `token-query-api-heartbeat-failed` / `token-query-go-heartbeat-failed` 这两个 Alarm 的 `alarmActions` 指向这个 Topic
- [x] 4. `cdk diff` / `cdk deploy`
- [x] 5. 手动把某个 canary 停掉（`stop-canary`）或者临时改坏 `HealthCheckUrl` 参数，触发一次真实的 ALARM，确认真的收到邮件通知，再恢复

## Part 5 — EventBridge Scheduler 操练（业务无关）

目标：Part 7 的第二层兜底（定时对账）需要用到 EventBridge Scheduler，同样先业务无关地练一遍再落地。

- [x] 1. 先建一个测试用的目标 Lambda
...
- [x] 3. 等 5-10 分钟，去 CloudWatch Logs 确认按预期频率触发
...
- [x] 4. 练一下 one-time schedule，对比跟 recurring 的区别
...
- [x] 5. 练完删掉测试 Schedule 和测试 Lambda
...
验证策略（不用真的等 `cdk destroy` 自然失败，也不要去手动破坏真实 preview 资源）。**完整流程图 + 所有命令已经整理到 [docs/knowledge/preview-cleanup-flow.md](knowledge/preview-cleanup-flow.md)，这里只跟踪进度，不重复贴命令了。**

- [x] 1. 新建 SQS 队列 + DLQ + Redrive Policy（CDK）—— **已部署**（`cdk deploy token-query-preview-cleanup`，2026-07-20 11:12 完成）
- [x] 2. 新建消费者 Lambda —— **已部署**
- [x] 3. 验证测试 A（进行中）—— 已发出测试消息，等重试周期跑完，详见 knowledge 文档「验证流程」第 1 节
- [x] 5. 确认 DLQ 告警 + 邮件通知，验证完记得 `purge-queue` 清理测试数据 —— 详见 knowledge 文档「验证流程」第 3/4 节
- [x] 6. 把 `cleanup-lambda-preview.yml` / `cleanup-go-preview.yml` 的失败分支接入实际发送 SQS 消息的步骤 —— **代码 + 权限都已部署**，队列已存在，这条链路现在是真正闭环的了
- [x] 7. 更新 `docs/cdk-deploy-commands.md`，补充这部分资源的部署/清理说明
  - 新增「PR Preview 清理兜底层（SQS + DLQ）生产部署」小节，说明依赖 `token-query-monitoring` 的 SSM 导出、必须先部署 monitoring 再部署这个 stack
  - 补了 monitoring 部署命令里之前漏掉的 `--parameters AlertEmail=...`（必填参数，没有默认值），以及首次部署后要去邮箱确认订阅的提醒
  - 「清理顺序」里加了 `token-query-preview-cleanup` 的 destroy 命令，放在 monitoring 前面（对应依赖关系，反向删除）
  - Stack Scope 表格补了 `monitoring` / `preview-cleanup` 两行

**Part 6 已全部完成**：三个栈（monitoring 的 SSM 导出、permissions 的新权限、preview-cleanup 全新栈）都已 `cdk deploy`，验证测试 A 确认消息能正常重试并最终进 DLQ，Alarm + 邮件通知也确认生效，部署文档已补充完整。
...
- [x] 1. 完成 Part 5 的 EventBridge Scheduler 操练
- [x] 2. 新建对账 Lambda（CDK 管理）：新建 `infra/cdk/lib/preview-reconciliation-stack.ts` + `infra/cdk/lambda/preview-reconciliation/index.js`，列出 CFN 栈 + Cloudflare Worker，比对 GitHub open PR 列表。`cdk synth` 已验证通过，完整设计见 [docs/knowledge/preview-cleanup-flow.md](knowledge/preview-cleanup-flow.md) 「第二层兜底」章节
- [x] 3. 用 EventBridge Scheduler 定时触发这个 Lambda —— `CfnSchedule`，`rate(1 day)`，代码已写好
- [x] 4. 对账发现的孤儿资源：孤儿 CFN 栈发消息到 Part 6 的队列复用同一套告警链路；孤儿 Cloudflare Worker 直接调 API 删除（Part 6 消费者不认识"删 Worker"这件事，没有硬塞进同一个消息格式）—— 代码已写好
- [x] 5. **部署 + 验证，交给你自己走**（我这边缺 Cloudflare Account ID、GitHub PAT、Cloudflare API Token 这几个输入，没法代劳）
  - 先手动创建两个 Secrets Manager 密钥（GitHub PAT + Cloudflare API Token）
  - `cdk deploy token-query-preview-reconciliation --parameters CloudflareAccountId=<你的账号id>`
  - 手动 `aws lambda invoke` 触发一次，看日志确认扫描结果
  - 想验证"真的能发现孤儿"：手动 `workflow_dispatch` 部署一个没有 open PR 对应的 Worker preview，再手动触发一次这个 Lambda，确认被识别成孤儿并删除
  - 完整命令见 [docs/cdk-deploy-commands.md](cdk-deploy-commands.md) 「Preview 环境定时对账层（第二层兜底）生产部署」
...
| Part 4 | SNS 告警通知落地（CDK） | Review + 验收 | 10–15 分钟（看 diff、确认收到测试告警邮件） | 31min |
| Part 5 | EventBridge Scheduler 操练（业务无关） | 动手操练 | 30–45 分钟 | 29min |
| Part 6 | SQS+DLQ 兜底落地（CDK，含消费者 Lambda + workflow 改造 + 双验证） | Review + 验收 | 30–45 分钟（review Lambda/IAM 代码、确认两种验证测试结果、确认 workflow 改动） | 95min |
| Part 7 第一层 | 去掉 3 个 cleanup workflow 的 paths 过滤 | Review | 已完成 | |
| Part 7 第二层 | 定时对账 Lambda + EventBridge Scheduler | Review + 验收 | 20–30 分钟（review 对账逻辑、确认孤儿资源场景验证结果） | 80min |
